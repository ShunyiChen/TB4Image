...
public class Scaler implements PlugIn, TextListener, FocusListener {
	...
	public static boolean averageWhenDownsizing = true;
	...
	public void scale(ImageProcessor ip, Overlay overlay) {
		if (newWindow) {
			Rectangle r = ip.getRoi();
			ImagePlus imp2 = imp.createImagePlus();
	...
	public void scale222(ImageProcessor ip, Overlay overlay) {
		if (newWindow) {
			Rectangle r = ip.getRoi();
